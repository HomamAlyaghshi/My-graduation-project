...
const Values = () => {
  ...
  return (
    <div className="w-full max-w-[1440px] py-[72px] flex flex-wrap items-center justify-center gap-[72px]">
      {valueItems.map((item, index) => (
        <div
          key={index}
          className="w-full max-w-[262px] h-auto p-[24px] flex flex-col items-center text-white bg-spaceMid rounded-3xl text-center shadow-md shadow-neon hover:shadow-white transition-all duration-500"
        >
          <img
            alt={item.title}
            src={item.image}
